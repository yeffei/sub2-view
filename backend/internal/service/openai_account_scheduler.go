...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	var stickyEscapeEvent *openAIStickyEscapeEvent
	if !req.StickyWeighted {
		...
		if escapedSticky != nil {
			stickyEscapeEvent = escapedSticky
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	account = s.service.applyResolvedPoolMemberOverrides(ctx, req.GroupID, req.Platform, account)
			...
			for _, stickyID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
				...
				for i, candidate := range ranked {
					if candidate.account != nil && candidate.account.ID == stickyID {
						...
						return buildOpenAIWeightedSelectionOrder(ordered, req), affinityHash, affinityIDs
					}
				}
			}
		}
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrder(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		fresh = s.service.applyResolvedPoolMemberOverrides(ctx, req.GroupID, req.Platform, fresh)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			continue
		}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	finishAttempt := func(attempt openAIAccountLoadSelectionAttempt) (*AccountSelectionResult, int, int, float64, string, []int64, map[string]int, error) {
		if attempt.err != nil {
			return nil, attempt.candidateCount, attempt.topK, attempt.loadSkew, attempt.cacheAffinityKeyHash, attempt.cacheAffinityTopKIDs, skipped, attempt.err
		}
		if attempt.result != nil {
			return attempt.result, attempt.candidateCount, attempt.topK, attempt.loadSkew, attempt.cacheAffinityKeyHash, attempt.cacheAffinityTopKIDs, skipped, nil
		}
		result, candidateCount, topK, loadSkew, affinityHash, affinityIDs, err := s.finishLoadBalanceSelectionFallback(ctx, req, attempt)
		return result, candidateCount, topK, loadSkew, affinityHash, affinityIDs, skipped, err
	}
	...
	if req.SubscriptionPriority {
		...
		if len(subscriptionAccounts) > 0 {
			...
			if (attempt.err != nil && (!attempt.noCompactCandidates || len(regularAccounts) <= 0)) || attempt.result != nil {
			...
			if len(regularAccounts) > 0 {
				...
				if (regularAttempt.err != nil && !regularAttempt.noCompactCandidates) || regularAttempt.result != nil {
					return finishAttempt(regularAttempt)
				}
				result, candidateCount, topK, loadSkew, affinityHash, affinityIDs, fallbackErr := s.finishLoadBalanceSelectionFallback(ctx, req, regularAttempt)
				if fallbackErr == nil && result != nil {
					return result, candidateCount, topK, loadSkew, affinityHash, affinityIDs, skipped, nil
				}
				subResult, subCandidateCount, subTopK, subLoadSkew, subAffinityHash, subAffinityIDs, subErr := s.finishLoadBalanceSelectionFallback(ctx, req, attempt)
				if subErr == nil && subResult != nil {
					return subResult, subCandidateCount, subTopK, subLoadSkew, subAffinityHash, subAffinityIDs, skipped, nil
				}
				return result, candidateCount, topK, loadSkew, affinityHash, affinityIDs, skipped, fallbackErr
			}
			return finishAttempt(attempt)
		}
	}
		...
		if freshLoadMap, loadErr := s.service.concurrencyService.GetAccountsLoadBatchFresh(ctx, loadReq); loadErr == nil {
			...
			if len(freshPlan.selectionOrder) > 0 {
				...
				if freshResult != nil {
					...
					attempt.cacheAffinityKeyHash = freshPlan.cacheAffinityKeyHash
					attempt.cacheAffinityTopKIDs = freshPlan.cacheAffinityTopKIDs
					return attempt
				}
...
func (s *defaultOpenAIAccountScheduler) finishLoadBalanceSelectionFallback(
	...
	req OpenAIAccountScheduleRequest,
	attempt openAIAccountLoadSelectionAttempt,
) (*AccountSelectionResult, int, int, float64, string, []int64, error) {
	candidateCount := attempt.candidateCount
	topK := attempt.topK
	loadSkew := attempt.loadSkew
	cacheAffinityKeyHash := attempt.cacheAffinityKeyHash
	cacheAffinityTopKIDs := attempt.cacheAffinityTopKIDs

	if len(attempt.selectionOrder) == 0 {
		return nil, candidateCount, topK, loadSkew, cacheAffinityKeyHash, cacheAffinityTopKIDs, noAvailableOpenAISelectionError(req.RequestedModel, attempt.compactBlocked)
	}

	if stickyFallback, stickyErr := s.tryFallbackToWeightedSticky(ctx, req); stickyErr != nil {
		return nil, candidateCount, topK, loadSkew, cacheAffinityKeyHash, cacheAffinityTopKIDs, stickyErr
	} else if stickyFallback != nil {
		return stickyFallback, candidateCount, topK, loadSkew, cacheAffinityKeyHash, cacheAffinityTopKIDs, nil
	}
	...
	for _, candidate := range attempt.selectionOrder {
		...
		}, candidateCount, topK, loadSkew, cacheAffinityKeyHash, cacheAffinityTopKIDs, nil
	}

	return nil, candidateCount, topK, loadSkew, cacheAffinityKeyHash, cacheAffinityTopKIDs, noAvailableOpenAISelectionError(req.RequestedModel, compactBlocked)
}
