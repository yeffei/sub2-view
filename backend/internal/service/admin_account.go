...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, anomalyReason, search string, groupID int64, privacyMode string, sortBy, sortOrder string) ([]Account, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
	accounts, result, err := s.accountRepo.ListWithFilters(ctx, params, platform, accountType, status, anomalyReason, search, groupID, privacyMode)
	if err != nil {
		return nil, 0, err
	}
...
func (s *adminServiceImpl) DeleteAccount(ctx context.Context, id int64) error {
	...
	return s.syncUpstreamPoolMembersForAccount(ctx, account, nil)
}
