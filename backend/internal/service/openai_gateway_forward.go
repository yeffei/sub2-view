...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	previousResponseIDPresent := gjson.GetBytes(originalBody, "previous_response_id").Exists()
	...
	if passthroughEnabled {
		setCacheInstrumentationOpenAIResponsesState(c, originalBody, promptCacheKey, false, previousResponseIDPresent)
	...
	for {
		...
		upstreamReq, upstreamTrace := traceOpenAIUpstreamHTTPRequest(upstreamReq)
		resp, err := s.httpUpstream.Do(upstreamReq, proxyURL, account.ID, account.Concurrency)
		recordOpenAIUpstreamHTTPTrace(c, upstreamTrace)
