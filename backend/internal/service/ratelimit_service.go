...
type AccountRuntimeBlocker interface {
	...
	OpenAIRoutingPoolMode5xxCooldown(ctx context.Context, groupID *int64, defaultCooldown time.Duration) time.Duration
}
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	if account.IsPoolMode() && !customErrorCodesEnabled {
		if s.handleOpenAIPoolModeServerError(ctx, account, statusCode, responseBody) {
			return true
		}
...
func (s *RateLimitService) handleOpenAIPoolModeServerError(ctx context.Context, account *Account, statusCode int, responseBody []byte) bool {
	...
	if reason == "" {
		reason = "pool mode upstream 5xx"
	}

	s.notifyAccountSchedulingBlocked(account, until, "pool_mode_5xx")
	if s.accountRepo == nil {
		slog.Warn("pool_mode_5xx_temp_unsched_repo_missing", "account_id", account.ID, "status_code", statusCode)
		return true
	}
	if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, reason); err != nil {
		slog.Warn("pool_mode_5xx_temp_unsched_set_failed", "account_id", account.ID, "status_code", statusCode, "error", err)
		return true
	}
	if s.tempUnschedCache != nil {
		if err := s.tempUnschedCache.SetTempUnsched(ctx, account.ID, state); err != nil {
			slog.Warn("pool_mode_5xx_temp_unsched_cache_set_failed", "account_id", account.ID, "error", err)
		}
	}
...
func hasRecoverableRuntimeState(account *Account) bool {
	...
	return hasActiveModelRateLimits(account.Extra, now) ||
		hasNonEmptyMapValue(account.Extra, "antigravity_quota_scopes")
}

func hasActiveModelRateLimits(extra map[string]any, now time.Time) bool {
	if len(extra) == 0 {
		return false
	}
	...
	for _, raw := range rawLimits {
		...
		resetAtRaw, _ := limit["rate_limit_reset_at"].(string)
