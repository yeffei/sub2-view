...
func TestTokenRefreshService_ProcessRefreshUsesOAuthRefreshCandidates(t *testing.T) {
	...
	require.ElementsMatch(t, []int64{1}, repo.updatedCredentialIDs)
	require.Zero(t, repo.clearTempCalls, "retry-cooldown candidates must not be refreshed or cleared")
}
