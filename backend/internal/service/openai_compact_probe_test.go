...
func TestBuildOpenAICompactProbeExtraUpdates_500NoCompactChannelDoesNotMarkUnsupported(t *testing.T) {
	now := time.Date(2026, 4, 10, 10, 0, 0, 0, time.UTC)
	...
	if _, exists := updates["openai_compact_supported"]; exists {
		t.Fatalf("did not expect openai_compact_supported for transient channel exhaustion")
	}
