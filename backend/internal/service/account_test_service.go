...
func (s *AccountTestService) runLightweightBackgroundTest(ctx context.Context, accountID int64, modelID string) (*ScheduledTestResult, bool, error) {
	...
	finishedAt := time.Now()
	result := &ScheduledTestResult{Status: "failed", StartedAt: startedAt, FinishedAt: finishedAt, CreatedAt: finishedAt}
	if check != nil {
...
func lightweightAccountProbeTarget(account *Account, modelID string) (accountProbeTarget, bool) {
	...
	return accountProbeTarget{AccountID: account.ID, Provider: provider, Model: model, Endpoint: endpoint, APIKey: apiKey, APIMode: accountProbeAPIMode(account), AuthScheme: accountProbeAuthScheme(account)}, true
}
