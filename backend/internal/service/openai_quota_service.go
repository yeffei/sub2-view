...
// OpenAIRateLimitResetCreditDetail is the sanitized metadata for one available reset credit.
...
func (s *OpenAIQuotaService) prepareUpstreamCall(ctx context.Context, accountID int64) (accessToken, chatGPTAccountID, proxyURL string, fedRAMP bool, err error) {
	...
	if account.IsShadow() {
		...
		if resolveErr != nil || resolved == nil {
			return "", "", "", false, infraerrors.Newf(http.StatusBadGateway, "OPENAI_QUOTA_SHADOW_RESOLVE_FAILED", "failed to resolve shadow account: %v", resolveErr)
		}
		account = resolved
	}
