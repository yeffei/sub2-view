...
type OpenAIGatewayService struct {
	...
	openAIRoutingPolicyCache sync.Map
	openAIRoutingMemberCache sync.Map
...
func (s *OpenAIGatewayService) detectCodexClientRestriction(c *gin.Context, account *Account, body []byte) CodexClientRestrictionDetectionResult {
	...
	if account != nil && account.IsCodexCLIOnlyEnabled() && s != nil && s.settingService != nil {
		...
		if s.settingService.IsOpenAIAllowClaudeCodeCodexPluginEnabled(ctx) {
			policy.Whitelist = append(policy.Whitelist, openai.ResolveAllowedClientEntries([]string{openai.AllowedClientClaudeCode})...)
		}
	}
	if account != nil {
		policy.Whitelist = append(policy.Whitelist, openai.ResolveAllowedClientEntries(account.GetCodexCLIOnlyAllowedClients())...)
	}
