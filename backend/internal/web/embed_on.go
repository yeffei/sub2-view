...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	cacheKey := requestOrigin(c.Request) + "|" + normalizeSEOPath(c.Request.URL.Path)
	...
	rendered := s.injectSettings(settingsJSON, c.Request)
...
func (s *FrontendServer) injectSettings(settingsJSON []byte, reqs ...*http.Request) []byte {
	...
	if len(reqs) > 0 && reqs[0] != nil {
		result = injectRouteSEO(result, resolveRouteSEO(reqs[0], settingsJSON))
	} else {
...
func serveSEOAsset(c *gin.Context) bool {
	switch c.Request.URL.Path {
	case "/robots.txt":
		c.Data(http.StatusOK, "text/plain; charset=utf-8", []byte(buildRobotsTXT(c.Request)))
		c.Abort()
		return true
	case "/sitemap.xml":
		c.Data(http.StatusOK, "application/xml; charset=utf-8", []byte(buildSitemapXML(c.Request)))
		c.Abort()
		return true
	default:
		return false
