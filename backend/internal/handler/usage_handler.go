...
// DashboardAPIKeysWorkbench handles compact workbench summaries for user's own API keys.
// POST /api/v1/usage/dashboard/api-keys-workbench
func (h *UsageHandler) DashboardAPIKeysWorkbench(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	usageStats, err := h.usageService.GetBatchAPIKeyUsageStats(c.Request.Context(), validAPIKeyIDs, time.Time{}, time.Time{})
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	errorStats, err := h.opsService.GetUserAPIKeyErrorSummaries(c.Request.Context(), subject.UserID, validAPIKeyIDs, time.Now().Add(-24*time.Hour))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	stats := make(map[int64]*APIKeyWorkbenchSummary, len(validAPIKeyIDs))
	for _, id := range validAPIKeyIDs {
		item := &APIKeyWorkbenchSummary{APIKeyID: id}
		if usage, ok := usageStats[id]; ok && usage != nil {
			item.TodayActualCost = usage.TodayActualCost
			item.TotalActualCost = usage.TotalActualCost
			item.SuccessRequests24h = usage.SuccessRequests24h
		}
		if errSummary, ok := errorStats[id]; ok && errSummary != nil {
			item.ErrorCount24h = errSummary.ErrorCount24h
			item.LatestError = errSummary.LatestError
		}
		item.AttemptCount24h = item.SuccessRequests24h + item.ErrorCount24h
		if item.AttemptCount24h > 0 {
			rate := float64(item.SuccessRequests24h) / float64(item.AttemptCount24h)
			item.SuccessRate24h = &rate
		}
		stats[id] = item
	}
