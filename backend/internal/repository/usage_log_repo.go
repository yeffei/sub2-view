...
func (r *usageLogRepository) GetBatchAPIKeyUsageStats(ctx context.Context, apiKeyIDs []int64, startTime, endTime time.Time) (map[int64]*BatchAPIKeyUsageStats, error) {
	...
	recent24hStart := endTime.Add(-24 * time.Hour)
	...
	query := `
		SELECT
			...
			COALESCE(COUNT(*) FILTER (WHERE created_at >= $5 AND created_at < $3), 0) as success_requests_24h
		FROM usage_logs
		WHERE api_key_id = ANY($1)
		  AND created_at >= LEAST($2, $4, $5)
		GROUP BY api_key_id
	`
	today := timezone.Today()
	rows, err := r.sql.QueryContext(ctx, query, pq.Array(normalizedAPIKeyIDs), startTime, endTime, today, recent24hStart)
	if err != nil {
		return nil, err
	}
	for rows.Next() {
		...
		var total float64
		var todayTotal float64
		var successRequests24h int64
		if err := rows.Scan(&apiKeyID, &total, &todayTotal, &successRequests24h); err != nil {
			_ = rows.Close()
			return nil, err
		}
		if stats, ok := result[apiKeyID]; ok {
			stats.TotalActualCost = total
			stats.TodayActualCost = todayTotal
			stats.SuccessRequests24h = successRequests24h
		}
	}
	if err := rows.Close(); err != nil {
		return nil, err
...
func (r *usageLogRepository) getEndpointStatsByColumnWithFilters(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8) (results []EndpointStat, err error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&row.CacheReadTokens,
			&row.CacheReadHitRequests,
			&row.CacheCreationRequests,
			&row.TotalTokens,
			&row.Cost,
			&row.ActualCost,
		); err != nil {
		...
		row.PopulateCacheDerivedMetrics()
		results = append(results, row)
	}
	if err := rows.Err(); err != nil {
		return nil, err
