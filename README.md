...
For the repository-local Docker workflow on Windows, use the PowerShell entry point under `deploy/`:

Use `http://127.0.0.1:8080` as the single local browser entry for this repo. Do not treat Vite ports such as `3000` as the site address; they are internal dev ports only.

```powershell
# Print or open the canonical local entry
powershell -ExecutionPolicy Bypass -File deploy/local-ops.ps1 entry
powershell -ExecutionPolicy Bypass -File deploy/local-ops.ps1 open-entry

# Keep 8080 fixed while switching between Vite dev mode and Docker prod mode
powershell -ExecutionPolicy Bypass -File deploy/local-entry.ps1 dev
powershell -ExecutionPolicy Bypass -File deploy/local-entry.ps1 prod
powershell -ExecutionPolicy Bypass -File deploy/local-entry.ps1 status

# Inspect the current bind scope and reachable local URL
powershell -ExecutionPolicy Bypass -File deploy/local-ops.ps1 check-entry

# Run a site smoke check
powershell -ExecutionPolicy Bypass -File deploy/local-ops.ps1 smoke -BaseUrl http://127.0.0.1:8080 -Email admin@example.com -Password admin123

# Rebuild frontend assets, rebuild the Linux hotfix binary, swap the container binary, restart, and run smoke checks
powershell -ExecutionPolicy Bypass -File deploy/local-ops.ps1 redeploy -BaseUrl http://127.0.0.1:8080

# Build a persistent local Docker image from current source and recreate the service
powershell -ExecutionPolicy Bypass -File deploy/local-ops.ps1 release-image -BaseUrl http://127.0.0.1:8080
```

`BIND_HOST=127.0.0.1` means local-only access. Change it to `0.0.0.0` in `deploy/.env` only if you intentionally want LAN or external exposure, then recreate the service with Docker Compose.
