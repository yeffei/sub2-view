...
const GroupSelectorStub = defineComponent({
  ...
  template: `
    <div data-testid="group-selector">
      <button type="button" data-testid="set-shadow-group" @click="$emit('update:modelValue', [7])">group</button>
    </div>
  `,
})
...
function buildOpenAISparkShadowAccount() {
  ...
  return {
    ...
    credentials: {
      ...
      compact_model_mapping: { 'gpt-5.3-codex-spark': 'gpt-5.3-codex-spark-compact' },
    },
    group_ids: [],
  } as any
}
...
function _buildAntigravityAccount(projectId = 'configured-project') {
  return {
    id: 3,
...
function _buildOpenAISetupTokenAccount() {
  return {
    ...buildAccount(),
    type: 'setup-token',
    extra: {
