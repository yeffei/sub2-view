<template>
  <div :class="flat ? 'usage-table-shell flex min-h-0 min-w-0 flex-1 flex-col' : 'card usage-table-shell flex min-h-0 min-w-0 flex-1 flex-col'">
    <div class="usage-table-scroll-x">
      <DataTable
        :columns="columns"
        :data="data"
        :loading="loading"
      ...
      >
        ...
        <template #empty>
          <div class="usage-ledger-empty-state">
            <span>账册暂无记录</span>
            <strong>{{ t('usage.noRecords') }}</strong>
            <p>{{ t('usage.noRecordsDesc', '当前筛选范围内还没有调用明细。可以切换时间范围、选择全部密钥，或重置筛选后再查看。') }}</p>
          </div>
        </template>
      </DataTable>
    </div>
      ...
      <div class="whitespace-nowrap rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-xs text-white shadow-xl dark:border-gray-600 dark:bg-gray-800">
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <template v-else-if="tooltipData && (!tooltipData.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN)">
...
.usage-table-scroll-x :deep(.table-wrapper) {
  width: 100%;
  max-width: 100%;
  min-width: 0;
  overflow-x: auto;
...
.usage-table-scroll-x :deep(table) {
  min-width: max-content;
}

.usage-ledger-empty-state {
  display: grid;
  place-items: center;
  gap: 0.5rem;
  min-height: 14rem;
...
.usage-ledger-empty-state span {
  color: #a67c52;
  font-size: 0.82rem;
  letter-spacing: 0.2em;
}

.usage-ledger-empty-state strong {
  color: #1f2320;
...
<style>
.dark .usage-ledger-empty-state span {
  color: #b89c74;
}

.dark .usage-ledger-empty-state strong {
  color: #f4efe4;
}

.dark .usage-ledger-empty-state p {
  color: #bdb5a8;
}
</style>
