...
const {
  ...
  getDashboardApiKeysWorkbench: vi.fn(),
...
describe('user KeysView column settings', () => {
  beforeEach(() => {
    ...
    getDashboardApiKeysWorkbench.mockReset()
    ...
    getDashboardApiKeysWorkbench.mockResolvedValue({ stats: {} })
  ...
  it('keeps filters and selected page size when sorting by current concurrency', async () => {
    ...
    await selects[1].vm.$emit('update:modelValue', 42)
    await flushPromises()
    await selects[2].vm.$emit('update:modelValue', 'active')
    await flushPromises()
